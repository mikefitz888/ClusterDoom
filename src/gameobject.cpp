...
namespace gameobject {
    ...
    Collision* GameObject::getCollision() { return &this->collision_profile; }

    void GameObject::setX(int x_) { position.x = x_; }
    void GameObject::setY(int y_) { position.y = y_; }
    void GameObject::setPosition(int x, int y) { setX(x); setY(y); }
    void GameObject::setJitter(int x, int y) {
        jitter_offset.x = x;
        jitter_offset.y = y;
    }
    Point<int> GameObject::getJitter() { return jitter_offset; }
